...
ingresosRouter.get(
  ...
  async (
    ...
    try {
      ...
      /*
       * Para la disponibilidad únicamente interesa la fecha final
       * consultada y no todas las operaciones del período.
       */
      const unidadesOcupadasFechaFin = [
        ...new Set(
          registros
            .filter(
              (registro) =>
      ...
      const unidadesDisponibles =
        await prisma.unidad.count({
          where: {
            estado: 'DISPONIBLE',

            ...(unidadesOcupadasFechaFin.length > 0
              ? {
                  id: {
                    notIn:
                      unidadesOcupadasFechaFin,
                  },
                }
              : {}),
          },
      ...
      res.status(200).json({
        ...
        message:
          'Información de ingresos obtenida correctamente',
        ...
        data: {
          ...
          porDia:
            Array.from(mapaDias.values()),

          porUnidad: Array.from(
            mapaUnidades.values(),
          ).sort((a, b) =>
            a.numeroUnidad.localeCompare(
              b.numeroUnidad,
            ),
          ),
